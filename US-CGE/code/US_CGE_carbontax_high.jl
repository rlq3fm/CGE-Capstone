...
samdir = joinpath(@__DIR__, "data", "US SAM_V2.csv")
...
policydir = joinpath(@__DIR__, "data", "carbon_policy_hightax.csv")
...
function generate_CGE()  
    ...
    ptinv = 1
    @variables m begin
        pprod[i = sectors], (start = pprod0[i]) # the production price
...
resdir = joinpath(@__DIR__, "..", "result", "Result-carbontax-high.csv")
