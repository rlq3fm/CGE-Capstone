#  A energy-economy CGE model, considering the energy-KL bundle
...
function solve_cge()  
    ...
    # fens ++ = fene
    @mapping(m, eq_fens[j in fens, i in sectors], qint[j, i] * parm[j] ^ sigmafe[i] - afens[j, i] * fene[i] * pfene[i] ^ sigmafe[i])
    @complementarity(m, eq_fens, qint[fens, sectors])

    @mapping(m, eq_pfene[i in sectors], pfene[i] ^ (1 - sigmafe[i])- sum(afens[j, i] * parm[j] ^ (1- sigmafe[i]) for j in fens))
